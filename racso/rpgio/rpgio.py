...
class Game():
    ...
    def printBoard(self):
        for r in range(self.size):
            if self.mode==1:
                ...
                print(" ".join(["\\ "+str(num).center(6)+" /" for num in range(r*self.size, (r+1)*self.size)]))
            else:
                print("".join([item for item in self.board[r]]) + "   " + str(r*self.size)+"-"+str((r+1)*self.size-1))
    ...
    def takenCells(self,initial):
        initialPos = self.xy(initial)
        taken = { initialPos }
        for direction in [(1,0),(-1,0),(0,1),(0,-1)]:
            for dif in range(1, self.size):
                pos = (initialPos[0]+dif*direction[0], initialPos[1]+dif*direction[1])
                if not(0<=pos[0]<self.size and 0<=pos[1]<self.size) or self.board[pos[1]][pos[0]]!=self.board[initialPos[1]][initialPos[0]]: break
                taken.add(pos)
        return taken

    def processCells(self,cells):
        element = self.board[cells[0][1]][cells[0][0]]
        amount = len(cells)
    ...
    def fillBoard(self):
        for r in range(self.size):
            for c in range(self.size):
                if self.board[r][c]== self.EMPTY: self.board[r][c] = self.items[rand(0,len(self.items)-1)]
    ...
    def run(self):
     ...
     ''')
        ...
        print("Each turn, you interact with a cell in the self.board PLUS every other cell of the same type horizontally or vertically connected with it.")
